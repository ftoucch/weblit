...
async def fulltext_check(
    ...
    try:
        ...
        try:
            request = FullTextCheckRequest(**data)
        ...
        async for event in fulltext_service.check_stream(request, user_id=user_id):
            await websocket.send_text(json.dumps(event, default=_serialize))

        await websocket.close()

    except WebSocketDisconnect:
        logger.info("Full text check WebSocket disconnected.")
    except Exception as e:
        logger.error(f"Full text WebSocket error: {e}")
        try:
            await websocket.send_text(json.dumps({
                "type": "error",
                "message": str(e)
