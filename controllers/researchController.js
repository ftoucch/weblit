...
const createResearch = async (req,res) => {
    ...
    if(!title || !description) throw new UnAuthenticatedError('please enter all field')
    const systematicReview = await SystematicReview.create({title, description, user})
    res.status(StatusCodes.CREATED).json({message: "Systematic Literature Review Created sucessfully"})
}
